...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="py-2"><span class="text-muted fw-light">{{ $modelName }}/</span> {{ $model ? 'Update' : 'Create' }}
        </h4>
        <div class="card">
            ...
            <form class="m-t-40" method="post"
                action="{{ $model ? route($routeUpdate, ['slug' => $params ?? '', Str::lower($modelName) => $model->id]) : route($routeStore, $params ?? '') }}"
